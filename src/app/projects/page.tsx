          ...
          <AvatarGroup translate="-6%" sideOffset={10}>
            {project.contributors.map((contributor) => (
              <Avatar
                ...
                className="h-8 w-8 border mr-0.5"
              >
                <AvatarImage src={contributor.avatar} alt={contributor.name} />
                <AvatarFallback className="bg-primary text-white text-xs">
                  {getInitials(contributor.name)}
...
export default function ProjectsPage() {
  const [projectsTextMask, setProjectsTextMask] = useState<string | undefined>(
    undefined,
  );
  const [selectedCategory, setSelectedCategory] = useState<Category>("all");
  ...
  const allTags = useMemo(() => {
    ...
    projects.forEach((p) => p.categories?.forEach((c: string) => tags.add(c)));
  ...
  useEffect(() => {
    ...
    ctx.fillText("PROJECTS", 50, 50);

    const dataUrl = canvas.toDataURL("image/png");
    requestAnimationFrame(() => setProjectsTextMask(dataUrl));
  }, []);
        ...
        <section className="relative max-w-7xl mx-auto z-20 -mt-24 bg-neutral-50 rounded-3xl p-4 md:p-8 mb-24 shadow-lg border">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-8">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-lg md:text-xl max-w-2xl opacity-95 leading-relaxed font-serif">
                Explore selected projects and see who built them, from internal
                initiatives to collaborations with industry partners.
              </p>
            </div>

            <DropdownMenu>
              <DropdownMenuTrigger asChild disabled={isLoading || isError}>
          ...
          {isLoading ? (
            <div className="w-full flex flex-col items-center justify-center py-24 text-secondary-gray">
